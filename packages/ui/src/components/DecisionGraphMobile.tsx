  ...
  return (
    ...
    >
          ...
          >
            ...
            >
              <span
                ...
                className={cn(
                  "flex-none rounded-(--radius-rc-sm) border px-1.5 py-0.5 font-(--font-rc-mono) text-(--text-rc-xs)",
                  // Step number gets a green-tinted pill for completed +
                  // current ("available / next") nodes; locked stays muted.
                  node.status === "completed" || node.status === "current"
                    ...
                    : "border-(--color-rc-border) bg-(--color-rc-surface) text-(--color-rc-text-muted)",
                )}
              >
      ...
      {nodes.length > 0 ? (
        ...
        >
          <Sparkles size={12} className="text-(--color-rc-icon-accent)" />
          End of journey
        </li>
      ) : null}
    </ol>
