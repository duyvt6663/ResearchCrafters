...
- `categories:performance` is the only `error` gate (minScore 0.7). Everything
...
| --- | --- | --- |
| `categories:performance` | error >= 0.7 | Floor that catches a 20%+ regression without flagging dev-mode noise. Tighten to 0.85 once we baseline against `next start` with a warm Prisma pool. |
